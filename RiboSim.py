...
if len(sys.argv) != 5:
    print('Usage: RiboSim.py read_count transcript_length initiation_rate_avg initiation_rate_sd')
    exit()
...
initiation_rate_sd = int(sys.argv[4])
...
for j in range(len(read_array)):
    ...
    for i in range(len(read_array[j]) + rand_int):
        #Calculate new initiation rate, variance uses normal distribution
        while initiation_step < 1:
            initiation_step = round_half_up(random.normalvariate(initiation_rate_avg, initiation_rate_sd))
